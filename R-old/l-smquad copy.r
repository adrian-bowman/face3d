...
smquad.face3d <- function(x, pdist, y, lambda, df = 5, method = "df",
    ...
    B <- bbase(x, xl = min(x), xr = max(x), nseg = nseg, deg = bdeg)
    m <- dim(B)[2]
    B <- cbind(B, sweep(B, 1, pdist, "*"), pdist^2)
    ...
    if (lambda.select) {
       if (method == "df") {
          lambda.df <- function(lambda, btb, bty, P) {
             ...
             mat <- matrix(0, nrow = 2 * m + 1, ncol = 2 * m + 1)
    ...
    if (!missing(fixed) && all(!is.na(fixed))) {
      ...
      #  A <- cbind(A, sweep(A, 1, pdist, "*"), pdist^2)
    ...
    pp <- list(x = x, pdist = pdist, y = y, nseg = nseg,
               ...
               fitted = mu, btb = btb, P = P, bty = bty, df = edf, curvature = 0.5 * cvture)
    return(invisible(pp))
}
