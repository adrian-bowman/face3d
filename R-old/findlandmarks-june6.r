...
for ( i in 1:4) {
 ...
 ## Now that first four landmarks have stabilized, begin the next landmark sequentially  
      ...
      # first initial guess of prior on face
                
 
   for (iter in 1:niter){
   g5          <- gpa$mean[c("pn","se", "enL", "enR", "sn" ), ]
   g5mean      <- apply(g5, 2, mean)
   covmat5     <- cov(matrix(c(aperm(sweep(glmks[c(1,5,9,10,4),,], 1:2, 
                           gmean[c(1,5,9,10,4),]), c(2, 1, 3))), nrow = n, byrow = TRUE))
 
   
   lmks5              <- rbind(face$lmks[c("pn", "se", "enL", "enR"), ], gpa$mean["sn", ])
   rownames(lmks5)[5] <- "sn"   
   id                 <- rownames(lmks5)[5]
   lmean              <- apply(lmks5, 2, mean)
   opa                <- procOPA(g5, lmks5, scale = FALSE)
   face$lmks          <- sweep(sweep(face$lmks,   2, lmean) %*% opa$R, 2, g5mean, "+")
   face$coords        <- sweep(sweep(face$coords, 2, lmean) %*% opa$R, 2, g5mean, "+")
      
      #picking out 5 landmarks
       i<-5
      lmks.cond         <- rbind(face$lmks[c("pn", "se", "enL", "enR"), ], gpa$mean["sn", ])      	           
      lmk               <- lmks.cond[i, ]
      ind               <- (i - 1) * 3 + 1:3
      covmat.i          <- covmat5[ind, ind]                         
      cond.cov          <- covmat.i - covmat5[ind, -ind] %*% solve(covmat5[-ind,-ind]) %*%covmat5[-ind, ind]   
      cond.mean         <-  c(t(g5))[ind] +  covmat5[ind, -ind] %*% solve(covmat5[-ind,-ind]) %*% 
                            (c(t(lmks.cond[-i,]))  - c(t(g5))[-ind] ) 
      crds              <- sweep(face$coords, 2, cond.mean)
      M.dst             <- rowSums((crds %*% solve(cond.cov)) * crds)
      crds              <- sweep(crds, 2, cond.mean, "+")
      ind               <- (M.dst <= 20)
      ...
      face              <- index.face3d(face, subset = ind)  
      sbst              <- subset.face3d(face, ind)           	           
      ql                <- quadlocate(face=sbst, si = -.25, si.within = .3, pair = FALSE,
                           constraint=NA, graphics = FALSE)
      loglik            <- ql$rss    	           
      logprior          <- logprior[ql$indices2]                                
      logpost           <- loglik + logprior
