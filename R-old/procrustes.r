"procrustes.face3d" <- function(lmks, scale = TRUE, threshold = 0.03,
  ...
  if (!require(rgl)) stop("procrustes.face3d requires the rgl package.")
...
## rotate to frontal
  ...
  PSC.i.all.rot <- apply(PSC.i.all.rot, 3,  function(x){
                         ROT.shape <- rotate3d(x,         angles[1], 0, 0, 1)
                         ROT.shape <- rotate3d(ROT.shape, angles[2], 1, 0, 0) 
                         ROT.shape <- rotate3d(ROT.shape, angles[3], 0, 1, 0)
                         ROT.shape
    })
