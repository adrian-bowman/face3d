findlandmarks.face3d <- function(face, lmks, orient = TRUE, monitor = FALSE, graphics = monitor,
   ...
   #---------------------------------------------------------------------------------
      ...
      initial <- any(is.na(c(face$lmks[c("pn", "enL", "enR", "se"), ])))
      if (monitor & initial) cat("Locating initial landmarks ... ")
      ...
      while (!flag & (i < nrow(nrst))) {
         ...
         if (length(which(crv > 0.05)) > 10) flag <- TRUE
   ...
   # if (monitor & initial) cat("completed.\n")

   #---------------------------------------------------------------------------------
   #                       Find a suitable registration
   #---------------------------------------------------------------------------------
   
   if (monitor) cat("Finding a suitable registration ...")

   load("/Volumes/adrian/research/shape/Face3D/bayesian-faces/lmks.dmp")
   # load("/Users/adrian/Desktop/bayesian-faces/lmks.dmp")
   ...
   rotate <- function(par, face, lmks.ref, gmean, covmat, graphics = FALSE, optim = TRUE) {
      ...
      for (nm in lmks.ref) {
         ...
         crds.i   <- sweep(crds, 2, gmean[nm, ])
      	 ...
      	 psigma   <- if (nm == "pn") 2 else 1
      	 prior    <- exp(-rowSums((crds.i %*% solve(covmat.i)) * crds.i) / (2 * psigma^2))
      	 like     <- exp(-face$rss)
      	 like[is.na(like)] <- 0
   ...
   lmks.ref <- c(      "enL", "enR", "se", "acL", "acR")
   trace <- as.numeric(monitor)
   if (monitor) cat("\n")
   rotate(rep(0, 6), face, lmks.ref, gmean, covmat, graphics = graphics)
   return(face)
   
   opt <- optim(rep(0, 6), rotate, face = face, lmks.ref = lmks.ref, gmean = gmean, covmat = covmat,
                method = "L-BFGS-B", lower = c(rep(-0.4, 3), rep(-5, 3)), upper = c(rep(0.4, 3), rep(5, 3)),
                control = list(trace = trace, fnscale = -1, maxit = 50))
   # print(opt$par)
