closest.face3d    <- function(x, shape, nearest = 30){
   ...
   sbst  <- subset.face3d(shape, order(dst)[1:min(nearest, length(dst))], remove.singles = FALSE)
   ...
   ind   <- which(L1 & L2 & L3)
   if (length(ind) > 0) {
      x1  <- XX[ind, 1] * PX[ind, ] + XX[ind, 2] * PY[ind, ] + sbst$coords[trpls[ind, 1], ]
   	  pts <- rbind(pts, x1)
   	  trp <- matrix(as.numeric(rownames(sbst$coords))[c(trpls[ind, ])], ncol = 3)
   	  id  <- rbind(id, trp)
   }
   
   pts   <- matrix(c(pts), ncol = 3)
