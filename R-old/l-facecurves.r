...
facecurves.face3d <- function(face, curves, graphics = FALSE, monitor = FALSE, pcrv.path = FALSE, start.pc = NA) {
...
# Col Names of matrix - curve name, lmk1, lmk2, df, si.target, resampled points/npts , boundary1, boundary2, perp.dist.bound,   distance, rotation range
   curves.df    <- matrix(c("nasal root left",            "se",  "enL",  0,   NA, 50,  0.1,    0.3,    6,  5, pi/2,
                            ...
                            "nasal bridge left",          "pn",  "acL",  5,  0.5, 40,  0.2,    0.4,   7,  5, pi/2,
                            "nasal bridge right",        "acR",   "pn",  5,  0.5, 40,  0.2,    0.4,   7,  5, pi/2,
                            ...
                            "mandible right",            "oiR",   "gn",  0,  0.5, 250, 0.1,    0.25,  12, 10, pi/2,
                            "mandible left",              "gn",  "oiL",  0,  0.5, 250, 0.1,    0.25,  12, 10, pi/2,
                            "Mandible",                  "oiR",  "oiL",  5,  0.5, 500, 0.2,    1.0,   10, 15, pi/2,
                            "upper eye socket right",    "exR",  "enR",  9, -0.5, 10,  0.4,    1.0,   20,  5, pi/2,
                            "upper eye socket left",     "enL",  "exL",  9, -0.5, 10,  0.4,    1.0,   20,  5, pi/2,
                            "lower eye socket right",    "exR",  "enR",  0,  0.5, 30,  0.4,    1.0,   15,  5, pi/2,
                            "les right",                 "exR",  "enR",  5, -0.5, 16,  0.4,    1.0,   15,  5, pi/2,
                            "lower eye socket left",     "enL",  "exL",  0,  0.5, 30,  0.4,    1.0,   15,  5, pi/2,
   ...
   # for (k in 1:nrow(face$lmks)) face$lmks[k, ] <- closest.face3d(face$lmks[k, ], face)$point
   ...
   for (i in 1:length(curves)) {
     ...
     if(length(grep("nasal bridge", curves[i])) > 0) {
            ...
            shape <- index.face3d(shape, distance=distance[i]) 
   	        ind1 <- sign(shape$shape.index)!=sign(si.target[i])
   	        ind11 <- which(cutt$closest.distance < 2) 
     	    shape$shape.index[ind11]       <- 0
            shape$kappa1[ind11]            <- 0
            shape$kappa2[ind11]            <- 0   
            shape$shape.index[ind1]        <- 0
 ...
 if (length(grep("mandible", curves[i])) > 0) {
 	        ref.path                       <- planepath.face3d(shape, lmks[1,], lmks[2,])$path
     	    xy                             <- closestcurve.face3d(shape, ref.path)
            lambda                         <- cumsum(c(0, sqrt(apply((diff(ref.path))^2, 1, sum))))
            xcoord                         <- lambda[xy$closest.curvept] # distance along the curve
            ycoord                         <- xy$closest.distance
            ind11                           <- ycoord < -20  # | ycoord > -10
            shape                          <- index.face3d(shape, distance=15)
            shape$shape.index[ind11]       <- 0
            shape$kappa1[ind11]            <- 0
            shape$kappa2[ind11]            <- 0
     ...
     if (length(grep("upper lip", curves[i])) > 0) {    
            ...
            # reference.path <- planepath.face3d(shape.smooth, lmks[1,], lmks[2,])$path
  	   }  	  
     ...
     if (length(grep("lower lip", curves[i])) > 0) {
   	...
   	if (length(grep("brow ridge left", curves[i])) > 0) {
            ...
            ref.path                       <- planepath.face3d(shape, lmks[1, ], lmks[2, ], boundary = boundary[i, ], 
                                              ...
                                              si.target[i], graphics = graphics)$path
            ...
            if (graphics)	 	           display.face3d(shape, colour = 6 + values)
       }
     ...
     if (length(grep("brow ridge right", curves[i])) > 0) {
            ...
            shape.smooth <- shape                     
            values                         <- pmax(abs(shape.smooth$kappa1), abs(shape.smooth$kappa2)) 
            if (graphics)	 	           display.face3d(shape.smooth, colour = 6 + values)
     ...
     if (length(grep("lower eye socket", curves[i])) > 0) {
   	    ...
   	    if (length(grep("les left", curves[i])) > 0) {
            ...
            for (k in 1:nrow(pcurve)) pcurve[k, ] <- closest.face3d(pcurve[k, ], face)$point
            reference.path                 <- pcurve
             if (graphics) { display.face3d(shape, colour= 2 + values)
   	                         spheres3d(lmks, col = "black", radius = 1)
                             spheres3d(reference.path, col = "black", radius = 0.5)}
     ...
     if (length(grep("upper eye socket", curves[i])) > 0) {
            reference.path                 <- planepath.face3d(shape, lmks[1, ], lmks[2, ], boundary = boundary[i, ], 
                                                               distance = distance[i], ngrid=50, graphics = FALSE)$path    
            ...
            shape.smooth                   <- subset.face3d(shape, !indd)
   	        values                         <- pmax(abs(shape.smooth$kappa1), abs(shape.smooth$kappa2))
   	        if (graphics)                  display.face3d(shape.smooth, colour= 2 + values)   	   
   	  }
      ...
      if ((df[i] == 0) && !is.na(si.target[i]))
         ...
         path <- planepath.face3d(shape, lmks[1, ], lmks[2, ], boundary = boundary[i, ],
                                  distance = distance[i], rotation.range=rotation.range[i], graphics = graphics)$path
      else
         path <- smoothpath.face3d(shape, shape.smooth, lmks[1, ], lmks[2, ], df = df[i], reference.path = reference.path, 
                                   si.target = si.target[i], rotation.range=rotation.range[i], curve.name = curves[i], boundary = boundary[i, ],
                                    npts = npts[i], distance = distance[i], perp.dist.bound = perp.dist.bound[i], graphics = graphics, 
