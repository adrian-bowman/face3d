"rotate.face3d" <- function(shape, id.lndms, rotation) {
    if (missing(rotation))  {
    ...
    if (rotation == "coronal" | rotation == "sagittal sinister" | rotation == "sagittal dexter" | rotation == "transversal"){
       ...
       ROT.shape <- rotate3d(ROT.shape,ANGLE.xz.mid,0,0,1)
       ROTlndms <- ROT.shape[lndms.two.mid,]
       ROTlndms.vec <- apply(ROTlndms,2,diff)
       ...
       ROT.shape <- rotate3d(ROT.shape,ANGLE.xz.mid,1,0,0)
       ROTlndms <- ROT.shape[lndms.two.lat,]
       ROTlndms.vec <- apply(ROTlndms,2,diff)
       ANGLE.xz.lat <- -1*as.numeric(atan2(ROTlndms.vec[3],ROTlndms.vec[1]))
       angles[3] <- ANGLE.xz.lat
       ROT.shape <- rotate3d(ROT.shape,ANGLE.xz.lat,0,1,0)
       names(angles) <- c("coronal z","coronal x","coronal y")
       ROT <- list(ROT.shape = ROT.shape, angles = angles)
           if (class(shape1) == "face3d") {
               shape1$coords <- rotate3d(shape1$coords,angles[1],0,0,1)
               shape1$coords <- rotate3d(shape1$coords,angles[2],1,0,0)
               shape1$coords <- rotate3d(shape1$coords,angles[3],0,1,0)
               shape1$lmks <- ROT.shape
               if ("curves" %in% names(shape1)) {
               shape1$curves <- rotate3d(shape1$curves,angles[1],0,0,1)
               shape1$curves <- rotate3d(shape1$curves,angles[2],1,0,0)
               shape1$curves <- rotate3d(shape1$curves,angles[3],0,1,0)
               }
               if ("mesh" %in% names(shape1)) {
               shape1$mesh <- rotate3d(shape1$mesh,angles[1],0,0,1)
               shape1$mesh <- rotate3d(shape1$mesh,angles[2],1,0,0)
               shape1$mesh <- rotate3d(shape1$mesh,angles[3],0,1,0)
               }
           }
    if (rotation == "sagittal sinister"){
       ROT.shape <- rotate3d(ROT.shape,pi/2,0,1,0)
       ROT <- list(ROT.shape = ROT.shape, angles = angles)
         if (class(shape1) == "face3d") {
               shape1$coords <- rotate3d(shape1$coords,pi/2,0,1,0)
               shape1$lmks <- ROT.shape
               if ("curves" %in% names(shape1)){
               shape1$curves <- rotate3d(shape1$curves,pi/2,0,1,0)
               }
               if ("mesh" %in% names(shape1)){
               shape1$mesh <- rotate3d(shape1$mesh,pi/2,0,1,0)
               }
               }
       }
    ...
    if (rotation == "transversal"){
       ROT.shape <- rotate3d(ROT.shape,pi/2,1,0,0)
       ROT <- list(ROT.shape = ROT.shape, angles = angles)
        if (class(shape1) == "face3d") {
               shape1$coords <- rotate3d(shape1$coords,-pi/2,1,0,0)
               shape1$lmks <- ROT.shape
               if ("curves" %in% names(shape1)){
               shape1$curves <- rotate3d(shape1$curves,-pi/2,1,0,0)
               }
               if ("mesh" %in% names(shape1)){
               shape1$mesh <- rotate3d(shape1$mesh,-pi/2,1,0,0)
               }
               }
       }
        #class(shape1) <- "face3d"
