...
smoothpath.ridge.face3d <- function(shape, shape.smooth, lmk1, lmk2, df = 5, distance = 5, curve.name = NA, penalty = .002,
                              reference.path = NA, rotation.range=rotation.range, si.target, boundary = c(0.2, 0.5), npts, graphics = graphics,        
...
#Project onto 2d surface and readying for smoothing procedure
   ...
   xcoord <- xcoord
   ycoord <- ycoord
   if (graphics){clr       <- topo.colors(20)[cut(values, 20, labels = FALSE)]
   plot(xcoord, ycoord, col = clr)
   abline(h = 0)}
   
   save(xcoord,ycoord,values,file="ridge_issue.dmp")
ridge           <- ridge2d.face3d(xcoord, ycoord, values, lambda = penalty, graphics = graphics)
