...
initialcurves.face3d <- function(face, curve.names,monitor = FALSE,
                                 graphics = FALSE, new.window = FALSE) {
   ...
   if (any(flag)) {
      for (curve.nm in curve.nms[flag]) {
         ...
         # ids  <- closest.face3d(face$lmks["se", ], sbst)$id
         ids  <- closest.face3d(face$lmks["pn", ], sbst)$id
         ...
         if (graphics) {
            ...
            spheres3d(ac, col = "yellow")
	         model  <- sm.psp(cbind(arcl, dst), -sbst2$kappa2, df = 12, ngrid = 100)
   ...
   if ("mid-line columella" %in% curve.names) {
      ...
      gc  <- gcurvature.face3d(pp0$path, 3, graphics = graphics)
      sn  <- gc$pos.max
      ...
      gc  <- gcurvature.face3d(pp2, 3, graphics = graphics)
   ...
   if ("nasal base left" %in% curve.names) {
      ...
      if (graphics) spheres3d(curves$"nasal base left", col = "red", radius = 0.5)
   }
   if ("nasal base right" %in% curve.names) {
      curves$"nasal base right" <-
         planepath.face3d(face1, face$lmks["acR", ], face$lmks["sn",])$path
      if (graphics) spheres3d(curves$"nasal base right", col = "red", radius = 0.5)
   }
