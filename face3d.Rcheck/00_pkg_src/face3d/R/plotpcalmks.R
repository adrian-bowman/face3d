...
plotpcalmks.face3d <- function(gpa, npc = 6, connect = list(),
        group = NA, control = NA, title = '3D PCA display', rgl.size = 2) {

  # if (dim(gpa$tan)[2] != length(group)) stop("mismatch in the size of gpa and group.")
...
shapes3d.pca.plot <- function(panel) {
   with(panel, {
      ...
      if (extremes.showing) {
         pc <- as.numeric(pc)
         shapes3d.plot.case(lower[, , pc], connect, col = "red", size = rgl.size)
         shapes3d.plot.case(upper[, , pc], connect, col = "blue",  size = rgl.size)
                      ...
                      # upper_pc4<- upper[, , 4]
                        # lower_pc5 <- lower[, , 5]
                      # upper_pc5 <- upper[, , 5]
                      # save(lower_pc2 , upper_pc2,
                              # lower_pc3 , upper_pc3,
                              # lower_pc4 , upper_pc4,
                              # lower_pc5 , upper_pc5, file="extremes_lmks.dmp")
                              #  save(lower,upper, file="extremes_mussels3.dmp")
...
shapes3d.plot.case <- function(shape, connect = list(), ...) {
   ...
   if (length(cpts) > 0)
      rgl::rgl.points(shape[cpts,1], shape[cpts, 2], shape[cpts, 3], ...)
   if (length(clines) > 0)
      rgl::rgl.lines(shape[clines, 1], shape[clines, 2], shape[clines, 3], ...)
   }
...
inv.tangcoord <- function(A, mu, approximate = TRUE) {
   ...
   if (!all(is.na(group))) {
      ...
      if (!is.na(control)) {
        closest    <- closestcontrol.face3d(gpa, npc, group, control)
   ...
   pca.display <- rpanel::rp.control(title, 
   ...
   rpanel::rp.checkbox(pca.display, mean.showing, shapes3d.pca.plot,
                     initval = TRUE, title = "Show mean")
   rpanel::rp.checkbox(pca.display, data.showing, shapes3d.show.data,
                     title = "Show data")
   rpanel::rp.doublebutton(pca.display, insp.case, 1, title = "",
                     range = c(1, dim(gpa$rotated)[3]), initval = 1,
                     action = shapes3d.inspect)
   if ((!all(is.na(group))) & (!is.na(control))) {
      rpanel::rp.checkbox(pca.display, control.mean.showing, 
                     shapes3d.pca.plot,
                     title = "Control mean")
      rpanel::rp.checkbox(pca.display, closest.showing, shapes3d.pca.plot,
                     title = "Closest control")
      }
   rpanel::rp.button(pca.display, action = shapes3d.scores.plot, "Scores plot")
   rpanel::rp.button(pca.display, action = shapes3d.scores.identify, "Identify points")
   # print(paste(1:npc, ' (', round(gpa$percent[1:npc]), '%)', sep = ''))
   rpanel::rp.radiogroup(pca.display, pc, 1:npc,
         labels = paste(1:npc, ' (', round(gpa$percent[1:npc]), '%)', sep = ''),
         action = shapes3d.pca.plot, title = "Principal components")
   rpanel::rp.button(pca.display, action = pc.animate, "Animate")
   rpanel::rp.checkbox(pca.display, spin.animate, title = 'Spin animation')
   rpanel::rp.checkbox(pca.display, extremes.showing, shapes3d.pca.plot, title = 'Show extremes')
   rpanel::rp.button(pca.display, action = shapes3d.tour, "Tour")
   rpanel::rp.slider(pca.display, speed, 0, 100, initval = 100, title = "Animation speed")
   rpanel::rp.doublebutton(pca.display, theta, -2, "Theta", action = rotate)
   rpanel::rp.doublebutton(pca.display, phi, -2,  "Phi", action = rotate)

   rgl::rgl.open()
   rgl::rgl.bg(color = c('white', 'black'))
   rgl::rgl.viewpoint(0, 0, fov = 1)
   rpanel::rp.do(pca.display, shapes3d.pca.plot)
