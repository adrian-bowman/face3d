orient.face3d <- function(face, ngrid = c(16, 4), angle.max = c(pi/4, pi/8),
   ...
   coords   <- sweep(face$coords, 2, centroid)
   normals  <- face$normals

   fn.nearest <- function(angles, dim = 1) {
   	if (abs(angles[1]) > 0) coords  <- rgl::rotate3d(coords,  angles[1], 1, 0, 0)
   	if (abs(angles[2]) > 0) coords  <- rgl::rotate3d(coords,  angles[2], 0, 1, 0)
   	if (abs(angles[3]) > 0) coords  <- rgl::rotate3d(coords,  angles[3], 0, 0, 1)
      rngy    <- range(coords[ , 2])
      ...
      if (graphics) {
         ...
         rgl::spheres3d(coords1[nearest, ], col = "green", radius = 2)
   ...
   normals <- normals + coords
   coords  <- rgl::rotate3d(coords,  angle1, 1, 0, 0)
   coords  <- rgl::rotate3d(coords,  angle2, 0, 1, 0)
   normals <- rgl::rotate3d(normals, angle1, 1, 0, 0)
   normals <- rgl::rotate3d(normals, angle2, 0, 1, 0)
   normals <- normals - coords
   
   if (graphics) {
      face1 <- list(coords = coords, triples = face$triples)
      plot(face, new = FALSE)
   }
   ...
   if (nlevels > 1) {
      ...
      normals <- normals + coords
      coords  <- rgl::rotate3d(coords,  angle2, 0, 1, 0)
      coords  <- rgl::rotate3d(coords,  angle3, 0, 0, 1)
      normals <- rgl::rotate3d(normals, angle2, 0, 1, 0)
