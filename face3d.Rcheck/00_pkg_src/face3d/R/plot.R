plot.face3d <- function (shape, display = "surface",
   ...
   if (length(rgl.dev.list()) == 0) new <- TRUE
   ...
   if ("direction" %in% substr(display, 1, 9)) {
      if (!("directions" %in% names(shape))) stop("directions not present.  These can be computed by index.face3d with the argument 'directions = TRUE'.")
      clr  <- if (length(colour) == 1 && is.character(colour) &&
                  !(substr(colour, 1, 7) %in% c("texture", "normal-"))) colour else "black"
    ...
    if (length(colour) == 1 && !is.na(colour) && colour == "shape index") {
        if ("shape.index" %in% names(shape)) {
            ...
            colour      <- rep("white", length(shape$shape.index))
            ind         <- !is.na(si)
            colour[ind] <- rgb(approx(brks, clr.r, xout = si[ind])$y,
                               approx(brks, clr.g, xout = si[ind])$y,
                               approx(brks, clr.b, xout = si[ind])$y)
            breaks      <- seq(-1, 1, by = 0.05)
            scaling <- "linear"
            col.palette <- rgb(approx(brks, clr.r, xout = breaks[-1] - 0.025)$y,
                               approx(brks, clr.g, xout = breaks[-1] - 0.025)$y,
                               approx(brks, clr.b, xout = breaks[-1] - 0.025)$y)
