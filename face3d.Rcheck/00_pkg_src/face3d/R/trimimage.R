trimimage.face3d <- function(shape, lmks.vertical = c("sn", "n"),
   ...
   shape <- rotate.face3d(shape, id.lndms = c(lmks.vertical, lmks.horizontal),
                         rotation = "coronal")
   ind <- shape$coords[ , 3] > min(shape$mesh[ , 3])
   ind <- ind & (shape$coords[ , 1] >= min(shape$mesh[ , 1]))
   ind <- ind & (shape$coords[ , 1] <= max(shape$mesh[ , 1]))
   ind <- ind & (shape$coords[ , 2] >= min(shape$mesh[ , 2]) - 0.1 * diff(range(shape$mesh[ , 2])))
   ind <- ind & (shape$coords[ , 2] <= max(shape$mesh[ , 2]) + 0.1 * diff(range(shape$mesh[ , 2])))
