subset.face3d <- function(x, ind, remove.singles = TRUE, ...) {
   ...
   sbst <- list(coords = matrix(c(shape$coords[ind, ]), ncol = 3,
   ...
   if ("axes" %in% names(shape))
      sbst$axes <- shape$axes[ind]
   if ("directions" %in% names(shape)) {
      sbst$directions <- shape$directions[ , , ind]
   }
