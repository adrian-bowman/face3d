planepath.face3d <- function(shape, x1, x2, pts1, pts2, direction, normal,
                         ...
                         bridge.gaps = FALSE, si.target, directions = FALSE,
								 graphics = FALSE, graphics.prompt = graphics) {
   ...
   x1 <- closest.face3d(x1, shape)$point
   if (!any(is.na(x2))) x2 <- closest.face3d(x2, shape)$point
   ...
   if (all(!is.na(boundary))) {
      ...
      if (length(which(ind1)) < 12)
         ...
         shape <- subset.face3d(shape, ind1)
   }
   ...
   if (!any(is.na(x2)) & !bridge.gaps) {
      ...
      if (nparts > 1) {
         ...
         for (i in 1:nparts) {
            ...
            d1[i]   <- closest.face3d(x1, shape.i)$distance
            d2[i]   <- closest.face3d(x2, shape.i)$distance
         }
   ...
   if (graphics) {
     ...
     if (graphics.prompt) cat("Press the Return key to see successive images.\n")
   ...
   if (as.numeric(graphics) == 2) rgl::spheres3d(crossings, radius = gspheres/5)
   ...
   if (graphics) {
   	  ...
   	  if (graphics.prompt) scan(quiet = TRUE)
   }
   ...
   if (graphics) {
      rgl::pop3d()
   	  if (si.target.present) display.face3d(shape, colour = 2 + values, new = FALSE)
   	  else                   display.face3d(shape, new = FALSE)
