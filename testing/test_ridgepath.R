...
ppath <- planepath(nose, x1, x2, si.target = 1, rotation = 0, integrated = TRUE, monitor = 2)
ppath <- planepath(nose, x1, x2, si.target = 1, integrated = TRUE, integrated.distance = 2, monitor = 2)
plot(template_male)
spheres3d(rbind(x1, x2), col = "yellow")
spheres3d(ppath$path, col = "blue", radius = 0.9)
nose_ridge <- ridgepath(nose, x1, x2, ppath, penalty = 0.002, si.target = 1, rotation = 0, monitor = 2)
