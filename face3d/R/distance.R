"distance.face3d" <- function(shape1, shape2) {
  ...
  dist.mat         <- shape2$vertices - shape1$vertices
  ...
  # k2               <- dim(shape1$vertices)[1]
  # index.dir        <- rep(1, k2)
  # index            <- which((shape1$vertices[,1] + shape2$vertices[,1]) / 2 <= 0)
  # index.dir[index] <- -1
  ...
  k2 <- dim(shape1$vertices)[1]
  #index.dir <- rep(1, k2)
  #index <- which((shape1$vertices[, 1] + shape2$vertices[, 1])/2 <= 
  #                 0)
  #index.dir[index] <- -1
  #sign.lr.x <- sign.lr.x * index.dir
  # dist.x <- sign.lr.x * abs(dist.mat[, 1])
