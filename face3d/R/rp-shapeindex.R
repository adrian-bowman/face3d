rp.shapeindex <- function(panel = TRUE, s = -1) {
   ...
   if (!requireNamespace("rpanel", quietly = TRUE)) stop("the rpanel package is not available.")
   ...
   surface.draw <- function(panel) {
      with(panel, {
         ...
         if (use.colours) {
            ...
            colr <- "green"
         rgl::persp3d(x, y, z, col = colr, aspect = "iso",
                   box = FALSE, axes = FALSE,
                   xlab = "", ylab = "", zlab = "", alpha = 1)
         rgl::persp3d(x, y, z, col = "black", aspect = "iso",
                   box = FALSE, axes = FALSE,
   ...
   if (panel) {
      pnl <- rpanel::rp.control(x = x, y = y, use.colours = TRUE,
                        clr = clr, clr.brks = clr.brks)
      # rp.slider(panel, k, rep(-1, 2), rep(1, 2), surface.draw, showvalue = TRUE)
      rpanel::rp.slider(pnl, s, -1, 1, surface.draw, showvalue = TRUE,
                labels = "Shape index")
      rpanel::rp.checkbox(pnl, use.colours, surface.draw, "use colours")
      rpanel::rp.do(pnl, surface.draw)
   }
   else {
