...
"slidingcurves.face3d" <- function(shape1,shape2, n = 100, type = "resample")  {
 ...
 curve.names     <- c(    "cheek-eye right",
   ...
   for (i in 1:40) {
        ...
        if(type=="resample"){
            k                       <- length(ind.i)
            curves2.optim[ind.i, ]  <- resample.face3d(shape2$curves[ind.shape2, ],n=k,threshold=1e-5)
        }else if(type=="resample/slide"){
