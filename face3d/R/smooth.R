smooth.face3d <- function(x, y, lambda, df, method = "df",
    ...
    if (missing(period)) period <- rep(NA, ndim)
...
ps.matrices <- function(x, xrange, ndims, nseg, bdeg = 3, pord = 2, period,
												decompose = TRUE) {
	...
	ndimx <- ncol(x)
	if (missing(period)) period <- rep(NA, ndimx)
