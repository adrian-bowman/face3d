# This code uses a fixed sigma and a simple product construction for the posterior

bayesian.landmarks <- function(lmks.initial, lmks.prior, monitor = 0) {
   ...
   while (del.lmks > 0.1 | del.Bhat > 0.1) {
      ...
      for (lmk.nm in rownames(lmks)) {
         ind   <- which(c(rdist(face$coords, t(lmks[lmk.nm, ]))) < 2)
         post  <- numeric()
         for (i in ind) {
            ...
            for (j in 1:nrow(lmks)) {
               dst   <- c(rdist(face$coords, t(lmks[j, ])))
               nhd   <- which(dst < 10)
               x1    <- dst[nhd]
